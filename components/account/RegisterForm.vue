...
export default {
  ...
  data: () => ({
    e1: 1,
    sexSelect: ['保密', '男', '女', '武装直升机', '其他'], // 你怎么能假定我的______
    respectSelect: [
      { des: '请用标准尊称称呼我', hint: '1', respectId: 1 },
      { des: '请以朋友的态度称呼我', hint: '2', respectId: 2 },
      { des: '请以最好的伙伴的态度称呼我', hint: '3', respectId: 3 } // 麻溜
    ],
    respectGroupSelect: ['溜溜梅'], // 它到底酸不酸? 你没事吧
    ...
    verifyCode: '',
    mailRule: [
      value => !!value || '邮箱 为必填项', // 没名没姓/api/v2
      (value) => {
        const emailPattern = /\w[-\w.+]*@([A-Za-z0-9][-A-Za-z0-9]+\.)+[A-Za-z]/
        if (emailPattern.test(value)) {
          return true
        } else {
          return false || '输入的邮箱不合法, 请检查' // 又逮着个用户名当邮箱使的, 阅读理解扣10分
        }
      }
    ],
    passwordRule: [
      value => !!value || '密码 为必填项' // 丧权辱账号
    ],
    nickNameRule: [
      value => !!value || '昵称 为必填项', // 没名没姓/api/v3
      (value) => {
        const nickNamePattern = /[A-Za-z0-9_\-\u4E00-\u9FA5]+/
        if (nickNamePattern.test(value)) {
          return true
        } else {
          return false || '昵称不合法, 请检查 (1-16个字符, 大小写A-z, 下划线_, 数字0-9, 中文字符, 横杠-)' // 这里以后要做一个敏感词检查
        }
      }
    ],
    verifyCodeRule: [
      value => !!value || '验证码 为必填项' // const userEmail = 'noreply@mihoyo.com'
    ]
  }),
  methods: {
    setPassword () {
